...
board ourName hm =
  S.svg [SP.width "400", SP.height "400", SP.version "1.1"] $
    [ S.rect [SP.x "0", SP.y "0", SP.width "400", SP.height "400"] [],
      ...
      <> [ S.text
             [SP.transform "translate(20 380)", SP.stroke "white"]
             [ Html.text
                 ( "x = " <> showText (round ourX :: Int)
                     <> ", y = "
                     <> showText (round ourY :: Int)
                 )
             ]
         ]
  where
    ...
    f (name, Ship pnt rotation _) =
      let (boardX, boardY) = boardPosition (position ourShip) pnt
       in shipPicture name rotation "red" (boardX, boardY)
...
shipPicture name rotation color (x, y) =
  S.g
    [SP.transform ("translate(" <> showText x <> " " <> showText y <> ")")]
    [ S.polygon
        [ points [(- w, -5), (0, 20), (w, -5)],
          SP.stroke color,
          SP.strokeWidth "5",
        ...
        [],
      S.text
        [SP.transform "translate(0 30)", SP.stroke "white"]
        [Html.text name]
    ]
  where
    w = 10
